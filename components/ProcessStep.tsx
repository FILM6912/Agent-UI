...
export const ProcessStep: React.FC<ProcessStepProps> = ({ step }) => {
  ...
  const getIcon = () => {
    switch (step.type) {
      ...
      case 'command': return <Sparkles className="w-4 h-4 text-indigo-500 dark:text-indigo-400" />;
  ...
  const getTitle = () => {
    ...
    switch (step.type) {
      case 'command': return 'Tool Execution';
           ...
           {step.type === 'command' && (
             <div className="mt-1 space-y-3 animate-in fade-in slide-in-from-top-1 duration-200">
                ...
                {(() => {
                  ...
                  const inputIndex = lines.findIndex(line => line.trim() === 'Input:');
                  ...
                  if (inputIndex !== -1) {
                    ...
                    inputContent = inputContent.replace(/```json\n?/g, '').replace(/```\n?/g, '').trim();
                  }
                              ...
                              {(() => {
                                try {
                                  ...
                                  return formatted.split('\n').map((line, i) => {
                                    ...
                                    line = line.replace(/"([^"]+)":/g, '<span class="text-cyan-500 dark:text-cyan-400">"$1"</span>:');
                                    // Color string values - Orange
                                    line = line.replace(/: "([^"]*)"/g, ': <span class="text-orange-500 dark:text-orange-400">"$1"</span>');
                                    // Color numbers - Green
                                    line = line.replace(/: (\d+\.?\d*)/g, ': <span class="text-green-600 dark:text-green-400">$1</span>');
                                    // Color booleans - Cyan (like keywords)
                                    line = line.replace(/: (true|false)/g, ': <span class="text-cyan-500 dark:text-cyan-400">$1</span>');
                                    // Color null - Cyan
                                    line = line.replace(/: (null)/g, ': <span class="text-cyan-500 dark:text-cyan-400">$1</span>');
                                    // Color brackets - Light gray
                                    line = line.replace(/([{}[\]])/g, '<span class="text-zinc-400 dark:text-zinc-500">$1</span>');
                                    
                                    return <div key={i} dangerouslySetInnerHTML={{ __html: line || '&nbsp;' }} />;
                                  });
                                } catch {
                                  return <span className="text-zinc-700 dark:text-zinc-300">{inputContent}</span>;
                                }
                              })()}
                            </pre>
                          </div>
                            ...
                            <Markdown 
                              ...
                              components={{
                                ...
                                code: ({className, children, ...props}: any) => {
                                  ...
                                  return isInline ? (
                                    <code className="bg-zinc-200 dark:bg-zinc-800 px-1 py-0.5 rounded text-[0.9em]" {...props}>{children}</code>
                                  ) : (
