...
import { Message, ChatSession, ModelConfig, AIProvider, MessageVersion, Attachment } from './types';
import { streamMessageFromGemini, generateChatTitle } from './services/geminiService';
...
export default function App() {
  ...
  const [settingsTab, setSettingsTab] = useState<'general' | 'account' | 'tools' | 'langflow'>('general');
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      // Ctrl + Q to open LangFlow settings
      if ((e.ctrlKey || e.metaKey) && (e.code === 'KeyQ')) {
        // Only open popup if we are already on the LangFlow settings page
        if (isSettingsOpen && settingsTab === 'langflow') {
          e.preventDefault();
          setIsLangFlowConfigOpen(true);
        }
      }
    };
  ...
  useEffect(() => {
    const targetPath = isSettingsOpen ? `/settings/${settingsTab}` : '/';
    if (window.location.pathname !== targetPath) {
      window.history.pushState({ path: targetPath }, '', targetPath);
    }
  }, [isSettingsOpen, settingsTab]);

  // Handle browser back/forward interactions
  useEffect(() => {
    const handlePopState = () => {
      const path = window.location.pathname;
      if (path.startsWith('/settings/')) {
        const tab = path.split('/')[2] as 'general' | 'account' | 'tools' | 'langflow';
  ...
  const [modelConfig, setModelConfig] = useState<ModelConfig>({
    ...
    enabledModels: ['gemini-3-flash-preview', 'gemini-2.5-flash-lite-latest', 'gpt-4o'],
  ...
  const history = useMemo(() => {
    return (Object.values(sessions) as ChatSession[]).sort((a: ChatSession, b: ChatSession) => b.updatedAt - a.updatedAt);
  }, [sessions]);
  ...
  const executeChatRequest = async (prompt: string, historyToUse: Message[], targetMessageId?: string, attachments?: Attachment[]) => {
    if (!targetMessageId) {
        setIsLoading(true);
    }
    ...
    try {
        const stream = streamMessageFromGemini(historyToUse, prompt, modelConfig, attachments);
        ...
        for await (const chunk of stream) {
            ...
            if (!messageInitialized) {
                const initialAssistantMsg: Message = {
                    ...
                    versions: [{ content: '', steps: chunk.type === 'steps' ? chunk.steps : undefined, timestamp: Date.now() }],
            ...
            if ((chunk.type === 'text' && chunk.content) || chunk.type === 'steps') {
                if (chunk.type === 'text' && chunk.content) {
                    accumulatedContent += chunk.content;
                }
  ...
  const handleSend = async (message: string, attachments: Attachment[] = []) => {
    ...
    const userMsg: Message = { 
      ...
      timestamp: Date.now(),
      versions: [{ content: currentPrompt, attachments: attachments, timestamp: Date.now() }],
      currentVersionIndex: 0
    };
    ...
    setSessions(prev => ({
      ...
      [activeChatId]: {
        ...prev[activeChatId],
        title: isFirstUserMessage ? currentPrompt.substring(0, 30) : prev[activeChatId].title,
    ...
    if (isFirstUserMessage) {
      generateChatTitle(currentPrompt).then(aiTitle => {
        setSessions(prev => ({ ...prev, [activeChatId]: { ...prev[activeChatId], title: aiTitle } }));
      }).catch(() => {});
    }

    await executeChatRequest(currentPrompt, historyBeforeNewMessage, undefined, attachments);
  };
  ...
  const handleEditUserMessage = async (messageId: string, newContent: string) => {
    ...
    const targetAssistantId = (nextMsg && nextMsg.role === 'assistant') ? nextMsg.id : undefined;

    setSessions(prev => {
        const session = prev[activeChatId];
        const updatedMessages = session.messages.map(msg => {
            if (msg.id === messageId) {
                const currentVersions = msg.versions || [{ content: msg.content, attachments: msg.attachments, timestamp: msg.timestamp }];
                const newVersion: MessageVersion = { content: newContent, attachments: msg.attachments, timestamp: Date.now() };
            ...
            if (msg.id === targetAssistantId) {
                 const currentVersions = msg.versions || [{ content: msg.content, steps: msg.steps, timestamp: msg.timestamp }];
                 const newVersion: MessageVersion = { content: '', timestamp: Date.now() };
    ...
    await executeChatRequest(newContent, historyToUse, targetAssistantId, originalMsg.attachments);
  };
  ...
  const handleRegenerate = async (messageId: string) => {
    ...
    const historyToUse = currentMessages.slice(0, msgIndex);
    const lastUserMsg = [...historyToUse].reverse().find(m => m.role === 'user');
    ...
    await executeChatRequest(lastUserMsg.content, historyToUse, messageId, lastUserMsg.attachments);
  };
  ...
  const handleNewChat = () => {
    ...
    setSessions(prev => ({ ...prev, [newId]: { id: newId, title: t('sidebar.newTask'), messages: [], updatedAt: Date.now() } }));
  ...
  const confirmDeleteChat = () => {
    ...
    setSessions(prev => {
      ...
      if (Object.keys(newSessions).length === 0) {
        const newId = crypto.randomUUID();
        setActiveChatId(newId);
        return { [newId]: { id: newId, title: t('sidebar.newTask'), messages: [], updatedAt: Date.now() } };
      }
      if (activeChatId === id) {
  ...
  const handleClearAllChats = () => {
      const newId = crypto.randomUUID();
      setSessions({ [newId]: { id: newId, title: t('sidebar.newTask'), messages: [], updatedAt: Date.now() } });
      setActiveChatId(newId);
      setPreviewContent(null);
  };
  ...
  return (
    <div className="flex h-screen w-screen bg-zinc-50 dark:bg-black text-zinc-900 dark:text-zinc-50 overflow-hidden font-sans relative transition-colors duration-200">
        ...
        >
          ...
          >
            <div className="p-6 text-center">
              ...
              <h3 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100 mb-2">{t('common.deleteTitle')}</h3>
      ...
      <LangFlowConfigModal 
        ...
        onSave={(url) => setModelConfig(prev => ({ ...prev, langflowUrl: url }))}
      />
    </div>
  );
}